...
# Template Frontend (HTML + Tailwind CSS + JavaScript)
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="id">
<head>
...
<body class="bg-gray-100 p-6">
    <div class="max-w-6xl mx-auto">
        <h1 class="text-3xl font-bold mb-6 text-gray-800">Product Information System</h1>
...
def create_product():
    ...
    try:
        ...
        cursor.execute(
            ...
            (data["sku"], data["name"], data["category"], data["price"], data["stock"], data.get("description", ""))
        )
        conn.commit()
        conn.close()
        return jsonify({"message": "Produk berhasil dibuat"}), 201
    except sqlite3.IntegrityError:
        return jsonify({"error": "SKU sudah terdaftar"}), 409
...
def update_product(product_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE products SET sku=?, name=?, category=?, price=?, stock=?, description=? WHERE id=?",
        (data["sku"], data["name"], data["category"], data["price"], data["stock"], data.get("description", ""), product_id)
    )
    conn.commit()
    conn.close()
